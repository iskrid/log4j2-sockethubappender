...
public final class NDC {
   ...
   * <p/>
   * <p>
   * The parent thread can obtain a reference to its diagnostic context using the {@link #cloneStack} method. It should
   * communicate this information to its child so that it may inherit the parent's diagnostic context.
   * <p/>
   * <p>
   ...
   * <p/>
   * <p>
   * As this is such an important problem in heavy duty systems and because it is difficult to always guarantee that the
   * remove method is called before exiting a thread, this method has been augmented to lazily remove references to dead
   * threads. In practice, this means that you can be a little sloppy and occasionally forget to call {@code remove}
   * before exiting a thread. However, you must call <code>remove</code> sometime. If you never call it, then your
   ...
   * <p/>
   * <p>
   * This method is a convenient alternative to multiple {@link #pop} calls. Moreover, it is often the case that at the
   * end of complex call sequences, the depth of the NDC is unpredictable. The <code>setMaxDepth</code> method
